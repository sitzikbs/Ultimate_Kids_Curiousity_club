...
def list_shows():
    ...
    try:
        ...
        if not shows:
            ...
            console.print("  [cyan]kids-podcast shows init <show-id> --template oliver[/cyan]")
            return
        ...
        for show in shows:
            ...
            theme_display = show.theme[:50] + "..." if len(show.theme) > 50 else show.theme
            table.add_row(
                show.show_id,
                show.title,
                theme_display,
...
def create_show(
    ...
    Examples:
      ...
      kids-podcast shows create my-show --title "My Show" --theme "Educational" --no-interactive
    """
    blueprint_manager = ShowBlueprintManager()
    ...
    if theme is None:
        if interactive:
            ...
            console.print("[red]Error: --theme is required in non-interactive mode[/red]")
            raise typer.Exit(1)
    ...
    if interactive:
        ...
        protagonist_values = typer.prompt("  Core values (comma-separated)", default="curiosity,creativity")
    ...
    from models.show import ConceptsHistory, Show, ShowBlueprint
    ...
    protagonist = Protagonist(
        ...
        voice_config={"provider": "mock", "voice_id": f"mock_{protagonist_name.lower().replace(' ', '_')}"},
    )
...
def init_show(
    ...
    template: str = typer.Option(..., "--template", "-t", help="Template name (oliver or hannah)"),
):
    ...
    try:
        ...
        console.print(f"\n[green]✓[/green] Show initialized from template: [bold]{template}[/bold]")
...
def edit_protagonist(
    ...
    name = typer.prompt(f"Name", default=blueprint.protagonist.name)
    age = typer.prompt(f"Age", default=str(blueprint.protagonist.age), type=int)
    description = typer.prompt(f"Description", default=blueprint.protagonist.description)
    
    values_str = ", ".join(blueprint.protagonist.values)
    new_values = typer.prompt(f"Core values (comma-separated)", default=values_str)
    values_list = [v.strip() for v in new_values.split(",")]
    
    catchphrases_str = ", ".join(blueprint.protagonist.catchphrases)
    new_catchphrases = typer.prompt(f"Catchphrases (comma-separated)", default=catchphrases_str)
    catchphrases_list = [c.strip() for c in new_catchphrases.split(",") if c.strip()]
    
    backstory = typer.prompt(f"Backstory", default=blueprint.protagonist.backstory or "")
...
def edit_world(
    ...
    try:
        ...
        if result.returncode == 0:
            console.print(f"[green]✓[/green] World file edited successfully")
        else:
...
def add_character(
    ...
    description: str = typer.Option(None, "--description", "-d", help="Character description"),
    interactive: bool = typer.Option(True, "--interactive/--no-interactive", help="Interactive mode"),
):
    ...
    Examples:
      ...
      kids-podcast shows add-character my-show --name "Bob" --role "Friend" --description "Helpful friend" --no-interactive
    """
    blueprint_manager = ShowBlueprintManager()
    ...
    if interactive:
        ...
        if not name or not role or not description:
            console.print("[red]Error: --name, --role, and --description are required in non-interactive mode[/red]")
            raise typer.Exit(1)
    ...
    character = Character(
        ...
        voice_config={"provider": "mock", "voice_id": f"mock_{name.lower().replace(' ', '_')}"},
    )
...
def suggest_topics(
    ...
    if blueprint.show.theme.lower().find("stem") >= 0 or blueprint.show.show_id.find("oliver") >= 0:
        # STEM suggestions
        suggestions = [
            "How do magnets work?",
    ...
    elif blueprint.show.theme.lower().find("history") >= 0 or blueprint.show.show_id.find("hannah") >= 0:
        # History suggestions
        suggestions = [
